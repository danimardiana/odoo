...
class SaleAdvancePaymentInv(models.TransientModel):
    ...
    def create_invoices(self):
        ...
        # down from here is SB's code , can be romoved
        sale_orders = self.env["sale.order"].browse(self._context.get("active_ids", []))
        for order in sale_orders.filtered(lambda x: not x.clx_invoice_policy_id):
            order._create_invoices(final=self.deduct_down_payments)
        if self.start_date and self.end_date:
            ...
            advance_lines = advance_lines.filtered(lambda x: x.start_date and x.start_date <= self.start_date)
            ...
            month_count = len(
                OrderedDict(
                    ((self.start_date + timedelta(_)).strftime("%B-%Y"), 0)
                    for _ in range((self.end_date - self.start_date).days)
                )
            )
            for i in range(0, month_count):
                for partner in partners:
                    ...
                    partner.with_context(
                        ...
                        start_date=self.start_date,
                        end_date=self.end_date,
                    ).generate_advance_invoice(adv_lines)
        else:
            count = 1
            for i in range(0, count):
                ...
                advance_sale_orders_sol = sale_orders.filtered(
                    ...
                    and (x.partner_id.child_invoice_selection == "sol" or x.partner_id.invoice_selection == "sol")
                )
                if advance_sale_orders_sol:
                    ...
                    for order in advance_sale_orders_sol:
                        order.partner_id.with_context(
                            create_invoice_from_wzrd=True, sol=True, order=order.id
                        ).generate_invoice()
