...
class Partner(models.Model):
    ...
    #         lambda sol: (not sol.end_date or (sol.end_date and sol.end_date >= today))
    #         and sol.start_date <= today
    #         and (not sol.last_invoiced or sol.last_invoiced.month != today.month)
    #     )
    #     if not so_lines:
    #         if self._context.get("from_generate_invoice"):
    ...
    #     prepared_lines = [line.with_context({"arrears": True})._prepare_invoice_line() for line in so_lines]
    ...
    #                         {invoice_lines[line["category_id"]]["name"], line["name"]}
    #                     )
    ...
    #     so_lines.write({"last_invoiced": today})
    #     order = so_lines[0].so_line_id.order_id
    #     vals = {
    #         "ref": order.client_order_ref,
    #         "type": "out_invoice",
    ...
    #         "invoice_user_id": order.user_id.id,
    #         "narration": order.note,
    #         "partner_id": self._context.get("co_op_invoice_partner", False)
    #         if self._context.get("co_op_invoice_partner", False)
    ...
    #             del x["line_type"]
    #             x["quantity"] = 1
    #         vals.update({"invoice_line_ids": [(0, 0, x) for x in prepared_lines]})
    #         self.env["account.move"].create(vals)
    #     if invoice_lines:
    #         vals.update({"invoice_line_ids": [(0, 0, x) for x in invoice_lines.values()]})
    #         self.env["account.move"].create(vals)
