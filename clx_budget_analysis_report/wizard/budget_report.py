    ...
    def get_budget_report_old(self):
        ...
        if yearly_subscription_lines:
            for sub in yearly_subscription_lines:
                if sub.end_date:
                    r = len(
                        OrderedDict(
                            ((sub.start_date + timedelta(_)).strftime("%B-%Y"), 0)
                            for _ in range((sub.end_date - sub.start_date).days)
                        )
                    )
                    if r == 0:
                        budget_month = 1
                    ...
                    budget_month = 12
                base = sub.analytic_account_id.recurring_invoice_line_ids.filtered(
                    lambda x: x.line_type == "base" and x.product_id.id == sub.product_id.id
                )
        ...
        for subscription_line in subscription_lines:
            if subscription_line.end_date:
                r = len(
                    OrderedDict(
                        ((subscription_line.start_date + timedelta(_)).strftime("%B-%Y"), 0)
                        for _ in range((subscription_line.end_date - subscription_line.start_date).days)
                    )
                )
                if r == 0:
                    budget_month = 1
            ...
            for i in range(0, budget_month):
                ...
                end_date_x = (
                    subscription_line.end_date
                    if subscription_line.end_date and subscription_line.end_date <= end_date_line
                    else end_date_line
                )
                ...
                vals = {
                    ...
                    "base_price": s_sum,
                    "end_date": subscription_line.end_date
                    if subscription_line.end_date and subscription_line.end_date <= end_date_line
                    else end_date_line,
                }
                ...
                available_line = report_data_table.search(
                    [
                        ("date", "=", current_month_start_date),
                        ("product_id", "=", subscription_line.product_id.id),
                        ("subscription_id", "=", subscription_line.analytic_account_id.id),
        ...
        if all_report_data:
            all_report_data.unlink()
        action = self.env.ref("clx_budget_analysis_report.sale_budget_report_action").read()[0]
        return action
